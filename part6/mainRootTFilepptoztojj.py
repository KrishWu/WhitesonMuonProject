    ...
    for event in tree:
        ...
        for i in range(numParticles - 1):
            for j in range (numParticles - 1):
                if (j >= i):
                    if (deltaM > abs(event.GetLeaf("Jet", "Jet.Charge").GetValue(i) - (-1 * event.GetLeaf("Jet", "Jet.Charge").GetValue(j + 1)))):
                        deltaM = abs(event.GetLeaf("Jet", "Jet.Charge").GetValue(i) - (-1 * event.GetLeaf("Jet", "Jet.Charge").GetValue(j + 1)))
                    ...
                    if (deltaM > abs(event.GetLeaf("Jet", "Jet.Charge").GetValue(i) - (-1 * event.GetLeaf("Jet", "Jet.Charge").GetValue(j)))):
                        deltaM = event.GetLeaf("Jet", "Jet.Charge").GetValue(i) - (-1 * event.GetLeaf("Jet", "Jet.Charge").GetValue(j))
                        m1.SetPtEtaPhiM(event.GetLeaf("Jet", "Jet.PT").GetValue(i), event.GetLeaf("Jet", "Jet.Eta").GetValue(i), event.GetLeaf("Jet", "Jet.Phi").GetValue(i), event.GetLeaf("Jet", "Jet.Charge").GetValue(i))
